...
const ThemeToggle = (ThemeToggleProps) => {
    ...
    return (
            ...
            {theme === 'dark' 
                ? (
                    <img onClick={() => setTheme( theme ==='dark' ? 'light' : 'dark')} className="h-5 xs:h-10 md:h-7" src={darkThemeIcon} alt="lightThemeIcon"/>
                    )
                : (
                    <img onClick={() => setTheme( theme ==='dark' ? 'light' : 'dark')} className="h-5 xs:h-10 md:h-7" src={lightThemeIcon} alt="darkThemeIcon"/>
                )
            }
        </div>  
    )
